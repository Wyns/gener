...
function setDropDownValue(dropdownId, valueDisplay, valueInInput){
    ...
    span.html(valueDisplay);
    span.removeClass('dis-none');
    input.addClass('dis-none');
...
function setDropDownValueOther(dropdownId, placeholder){
    var span = $("#"+dropdownId);
    var input = $("#"+dropdownId+"Input");
    span.addClass('dis-none');
    input.removeClass('dis-none');
...
function initPostcodeElasticOnBuildingPages(id) {
    ...
    postcodeInput.keyup(function() {
        if ($(this).val().length > 2){
            ...
            var dataToSend = { query: $(this).val(), is_link_js: true, timestamp: Date.now() , location: location, userBrowser: userBrowser};
            if (siteid != null) {
...
function initPostcodeElasticSelectAddress() {
    ...
    postcodeInput.keyup(function() {
        if ($(this).val().length > 2){
            ...
            var dataToSend = { query: $(this).val(), isJustSelect: true, timestamp: Date.now(), location: location, userBrowser:userBrowser };
    ...
    textBox.keyup(function() {
        ...
        if (value.length > 2){
            ...
            $.ajax({
                url: homeURL + "elasticsearch/",
                type: 'POST',
                data: { query: value, is_link_js: is_map, timestamp: Date.now(), location: location, userBrowser: userBrowser }
            }).done(function( data, status, xhr ) {
                if ( xhr.getResponseHeader('Timestamp') < timestamp ) return;
                timestamp = xhr.getResponseHeader('Timestamp');
...
function becomeChampionInit() {
    $("#becomeChampionForm").validate({ rules: { email: "validEmail" , checkboxAgree: { required:true }}, messages: { checkboxAgree: { required: "You must agree"} } });
    $("#pafid").change(function() {
        isOKBecomeChampion();
    });
}
function watchChampionForm(){
    if ($("#pafid").val() && $("#pafid").val().length > 0){
        $("#submitChampionForm").removeAttr("disabled");
    }
}
function becomeChampionSubmit() {
    if (!$("#becomeChampionForm").valid() || !$("#pafid").val())return;
    $("#submitChampionForm").attr("disabled","disabled");
    var dataToSend = {
